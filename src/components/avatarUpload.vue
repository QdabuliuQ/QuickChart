<template>
  <div class="avatarUpload">
    ...
    <input ref="inputRef" @change="change" style="display: none" type="file" accept="image/jpeg,image/jpg,image/png" />
  </div>
</template>
...
const change = async () => {
  ...
  if(fileType(file.name) !== 'image') return proxy.$notice({
    type: 'error',
    message: '图片类型只支持png/jpg/jpeg',
    position: 'top-left'
  })
  let size = file.size / 1024 / 1024;
  if(size > 3) return proxy.$notice({
    type: 'error',
    message: '图片大小不能超过3M',
    position: 'top-left'
  })
