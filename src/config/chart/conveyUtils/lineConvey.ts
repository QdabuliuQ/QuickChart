...
export const create = (config: any) => {
	...
	const xAxis = config.xAxis[0].data
...
export const convey = (
	...
	const rowsTLength = Object.keys(rows[0].cells).length
	...
	const rowsALength = Object.keys(rows).length - 1
	for (let i = 1; i < rowsALength; i++) {
		const rowsItemLength = Object.keys(rows[i].cells).length
