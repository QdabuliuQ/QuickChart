  ...
  <div id="paramsXasis">
    <div class="xasisContainer">
      <div class="asisTargetList">
        ...
        <div
          v-if="yAsisList.length < 2"
          @click="addAsisTarget"
          class="targetAddBtn"
        >
          <i class="iconfont i_plus"></i>添加Y轴
        </div>
      </div>
...
interface comInitData {
  yAsisList: { label: string; value: number }[];
  activeIndex: number;
  activeXasis: string;
  tempOption: any;
  allTempOption: any;
...
export default defineComponent({
  ...
  setup(props) {
    ...
    const addAsisTarget = () => {
      data.yAsisList.push({
        label: "Y轴" + (data.yAsisList[0].value + 2),
        value: data.yAsisList[0].value + 1,
      });
    ...
    const deleteItem = (i: number) => {
      if (data.yAsisList.length == 2) {
        data.yAsisList.splice(i, 1);
