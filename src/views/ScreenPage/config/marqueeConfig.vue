	...
	<div class="marquee-config">
		<template v-if="info">
			...
			<series-item title="文本内容">
				<el-input v-model="info.content" maxlength="100" size="small" />
			</series-item>
			<series-item title="滚动速度">
				<el-input-number v-model="info.speed" :max="10" :min="0.1" :step="0.1" size="small" />
			</series-item>
...
let stop1 = watch(
	...
	debounce(() => {
		if (update.value) {
			...
			screen.updateScreenOptionOfElementProperty(idx.value, 'content', (info.value as any).content)
			screen.updateScreenOptionOfElementProperty(idx.value, 'speed', (info.value as any).speed)
			screen.addOperationRecordItem(getRecordOption('modify', 'marquee'))
		}
		update.value = true
	}),
	{
